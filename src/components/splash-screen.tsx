...
export default function SplashScreen({ onComplete }: SplashScreenProps) {
    ...
    useEffect(() => {
        ...
        // Phase 2: Logo glows and pulses (800ms → 2200ms)
        const exitTimer = setTimeout(() => setPhase("exit"), 2200);
        // Phase 3: Exit animation (2200ms → 3000ms), then unmount
    ...
    return (
        <div
            className={`fixed inset-0 z-[100] flex flex-col items-center justify-center bg-[#0a0a1a] transition-all duration-700 ${phase === "exit" ? "opacity-0 scale-110" : "opacity-100 scale-100"
                }`}
        >
            {/* Ambient glow rings */}
            <div
                className={`absolute w-[500px] h-[500px] rounded-full transition-all duration-1000 ${phase === "enter"
                        ? "scale-0 opacity-0"
            ...
            />
            <div
                className={`absolute w-[800px] h-[800px] rounded-full transition-all duration-[1400ms] delay-200 ${phase === "enter"
                        ? "scale-0 opacity-0"
                        : "scale-100 opacity-100"
                    }`}
                style={{
            ...
            {/* Logo icon */}
            <div
                className={`relative mb-8 transition-all duration-700 ease-out ${phase === "enter"
            ...
            >
                <div className="relative h-24 w-24 rounded-2xl bg-gradient-to-br from-indigo-500 via-purple-500 to-indigo-600 flex items-center justify-center shadow-2xl shadow-indigo-500/40">
                    ...
                    {/* Glow effect */}
                    <div
                        className={`absolute -inset-1 rounded-2xl bg-gradient-to-br from-indigo-400 to-purple-500 blur-xl transition-opacity duration-700 -z-10 ${phase === "glow" ? "opacity-60 animate-pulse" : "opacity-0"
                            }`}
                    />
                </div>
            </div>
            ...
            {/* Logo text */}
            <div
                className={`transition-all duration-700 delay-200 ease-out ${phase === "enter"
            ...
            {/* Tagline */}
            <div
                className={`transition-all duration-500 delay-500 ease-out ${phase === "enter"
            ...
            <div className="absolute bottom-16 w-48 h-0.5 bg-white/5 rounded-full overflow-hidden">
                <div
                    className={`h-full bg-gradient-to-r from-indigo-500 to-purple-500 rounded-full transition-all ease-linear ${phase === "enter"
                            ? "w-0 duration-[800ms]"
                            : phase === "glow"
                                ? "w-3/4 duration-[1400ms]"
                                : "w-full duration-[800ms]"
                        }`}
                />
            </div>
        </div>
