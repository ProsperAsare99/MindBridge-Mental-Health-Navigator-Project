      ...
      <div className="space-y-6">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 gap-3">
            {[
              { label: "Yes, I have strong support", value: "strong" },
              { label: "Sometimes, but not always", value: "somewhat" },
              { label: "No, I feel alone", value: "alone" }
            ].map((opt) => (
              <button
                key={opt.value}
                onClick={() => update({ supportLevel: opt.value })}
                className={`rounded-[1.25rem] border-2 p-5 text-left text-sm font-black transition-all ${
                  data.supportLevel === opt.value 
